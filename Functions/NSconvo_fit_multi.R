...
{   
  ...
  function(s) {
    ...
    if(inherits(possibleError, "error")){
      ...
      if(!is.matrix(data)){
        tmp1 <- backsolve(cov.chol,  matrix(c(data[1:(length(data)/(2*t))],data[length(data)/2+1:(length(data)/(2*t))]),ncol=1), transpose = TRUE)
        #tmp1 <- backsolve(cov.chol, c(data)-c(rep(b0[1],dim(data)[1]/2),rep(b0[2],dim(data)[1]/2)), transpose = TRUE)
        ...
        for(tt in 1:nrow(data)){
          tmp1 <- backsolve(cov.chol,  matrix(c(data[tt,1:(dim(data)[2]/(2*t))],data[tt,dim(data)[2]/2+1:(dim(data)[2]/(2*t))]),ncol=1), transpose = TRUE)
          #tmp1 <- backsolve(cov.chol, c(data)-c(rep(b0[1],dim(data)[1]/2),rep(b0[2],dim(data)[1]/2)), transpose = TRUE)
