...
multivariate_matern_semi_explicit_regular_grid_v2 <-function(theta,wind,wind_var,time, locations){
  ...
  temploc1.temp <- temploc2.temp <- matrix(,ncol=nrow(coords),nrow=nrow(coords))
  
  for(rr in 1:nrow(coords)){
    for(ss in 1:nrow(coords)){
      xx <- c(coords[rr,1]-coords[ss,1],coords[rr,2]-coords[ss,2])
      temploc1.temp[rr,ss] <- xx[1]
      temploc2.temp[rr,ss] <- xx[2]
    }
  }
  
  temploc1 <- c(temploc1.temp)
  temploc2 <- c(temploc2.temp)
  
  SS <- list()
  S=matrix(NA,  q*nrow(coords)*time, q*nrow(coords)*time)
  
  for(i in 1:q){
    for(j in 1:i){
      
      temp2=(i-1)*(nrow(coords)*time)+1:(nrow(coords)*time)
      temp1=(j-1)*(nrow(coords)*time)+1:(nrow(coords)*time)
      ...
      if(i==j){
        ...
        for(tt in 1:t){
          ...
          Int.func <- function(c,hvec){   
            y.fun  <- function(y) y^(nu[i])*exp(-y)*dmvn(X=hvec, mu=(tt-1)*w,sigma=((tt-1)^2*Sigma+beta^2*2*y*diag(2)))
            sapply(c, y.fun)
          }
          lai <- function(x1,x2) integrate(Int.func, lower=0, upper=Inf, hvec=c(x1,x2))$val
          
          lai <- Vectorize(lai, vectorize.args = c("x1", "x2"))
          
          tempp <- lai(temploc1,temploc2)
          temploc <- matrix(4*pi*tempp*beta^2/gamma(nu[i]), ncol=nrow(coords), nrow=nrow(coords),byrow=T)
      ...
      if(i!=j){
        ...
        for(tt in 1:t){
          cat(tt,'\n')
          Int.func <- function(c,hvec){   
            y.fun  <- function(y) y^(nu3)*exp(-y)*dmvn(X=hvec, mu=(tt-1)*w,sigma=((tt-1)^2*Sigma+beta^2*2*y*diag(2)))
            sapply(c, y.fun)
          }
          lai <- function(x1,x2) integrate(Int.func, lower=0, upper=Inf, hvec=c(x1,x2))$val
          ...
          tempp <- lai(temploc1,temploc2)
          temploc <- matrix(sqrt(var[i] * var[j])*rho*4*pi*tempp*beta^2/gamma(nu3), ncol=nrow(coords), nrow=nrow(coords),byrow=T)
...
matern_random_cov_scratch <-function(theta, wind, wind_var, max_time_lag, q, new_locations, meters = T){
